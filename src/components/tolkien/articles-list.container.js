    ...
    render() {
        ...
        if(this.state.articles.length > 0 && this.state.catArticle) {
            ...
            let allArticles = this.state.articles.map((post, index) => {
                return (
                    <li key={"em-li-" + index}><Link to={"/tolkien/articles/" + post.id + "/" + post.slug}><span dangerouslySetInnerHTML={ {__html: post.title.rendered} } /></Link></li>
                )
            });
            ...
            return (
                <div className="small-12 medium-6 large-4 columns">
                    ...
                    <Link to={"/tolkien/" + catId + "/" + catSlug}>Tous les articles</Link>
                </div>
            );
